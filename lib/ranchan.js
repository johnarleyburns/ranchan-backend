...
function threadToCSVHeaders() {
    var csv = "";
    csv += "threadId,parentId,content,date,lastBump,chats,nsfw"
    csv += "\n\r";
    return csv;
}

function threadToCSV(thread) {
    var csv = "";
    csv += thread.threadId + ",";
    csv += thread.parentId + ",";
    csv += csvparser.quote(thread.content) + ",";
    csv += thread.date.getTime() + ",";
    csv += thread.lastBump.getTime() + ",";
    csv += thread.chats + ",";
    csv += thread.nsfw;
    csv += "\n\r";
    return csv;
...
exports.getThreadsCSV = function(parentId, callback) {
    exports.getThreads(parentId, function(err, threads) {
        if (err) {
            callback(err);
            return;
        }
        var csv = threadToCSVHeaders();
        threads.forEach(function(thread){
            csv += threadToCSV(thread);
        });
        callback(err, csv);
    });
}
...
function authorizedDeleteThread(threadId, parentId, callback) {
    Thread.findOneAndRemove({
        ...
        if (parentId === '0') { // delete children
            Thread.remove({parentId: threadId}, function(err) {
                callback(err, savedThread);
            });
            return;
        }
        Thread.findOne({
...
exports.deleteThread = function(threadId, parentId, auth, callback) {
    if (!auth.moderatorId) {
        var err = exports.newAuthError("Only a moderator can delete threads");
        callback(err);
        return;
    }
...
function authorizedDeleteThreadsForIP(ipAddress, callback) {
    Thread.remove({ipAddress: ipAddress}, callback);
    return;
}

exports.deleteThreadsForIP = function(ipAddress, auth, callback) {
    if (!auth.moderatorId) {
        var err = exports.newAuthError("Only a moderator can delete threads");
        callback(err);
        return;
    }
    authorizedDeleteThreadsForIP(ipAddress, callback);    
};
...
exports.postBan = function(ban, auth, callback) {
    if (!auth.moderatorId) {
    ...
    Thread.findOne({
        ...
        ban.threadId = thread.threadId;
        ban.content = thread.content;
        ban.moderatorId = auth.moderatorId;
        Ban.create(ban, callback);
    });
};

exports.postBanWithDelete = function(ban, auth, callback) {
    if (!auth.moderatorId) {
        callback(exports.newAuthError("Only a moderator can do that"));
        return;
    }
    exports.postBan(ban, auth, function(err, ban2) {
        if (err) {
            callback(err);
            return;
        }
        authorizedDeleteThreadsForIP(ban.ipAddress, function(err, threads) {
            if (err) {
                callback(err);
                return;
            }
